...
////=============================================================================================================================
  ...
  def Movecars(nodes: java.util.List[Rectangle],move:java.lang.Double):java.util.List[java.lang.Double] = {
    getX(nodes).map(mapspawn(move)).map(Double.box).asJava
  }
...
//==================================================================================================================================//
  ...
  def frogDeath(frog: Node, Lake: Rectangle, LogR:Rectangle,LogL:Rectangle,LogLL:Rectangle, carsR: java.util.List[Rectangle], carsL: java.util.List[Rectangle], restart: java.lang.Double, atual: java.lang.Double)={
    FrogDeath(frog, Lake, List(LogR,LogL,LogLL), carsR.asScala.toList,carsL.asScala.toList, restart, atual,Await.result(carstwoSideContact(frog,carsR.asScala.toList,Lake,carsL.asScala.toList),1000 seconds))
  }
  var carsSIDEContact=(frog:Node,cars: List[Rectangle],Lake:Rectangle)=>{
    Future[Boolean] {
      CheckLake (frog, Lake) match {
        case true => true
        case false => cars.map (MapContact (frog) ).reduce (DeathCar (frog) )
      }
    }
  }
  var carstwoSideContact:(Node, List[Rectangle],Rectangle,List[Rectangle]) => Future[Boolean] = (frog:Node,carsR: List[Rectangle],Lake:Rectangle,carsL: List[Rectangle])=> {
    for {
      r1 <- carsSIDEContact(frog, carsL, Lake)
      r2 <- carsSIDEContact(frog, carsR, Lake)
    } yield (r1 || r2)
  }
  ...
  var FrogDeath:( Node,Rectangle,List[Rectangle],List[Rectangle],List[Rectangle],Double,Double,Boolean)=> java.lang.Double= (frog: Node, Lake: Rectangle, Logs: List[Rectangle], carsR: List[Rectangle], carsL: List[Rectangle], restart: Double, atual: Double,carContact:Boolean) =>
    frog.getTranslateY  match {
      ...
      case _ => CheckLake(frog, Lake) match {
        case true => !(Logs.map(MapContact(frog)).reduce(DeathLogs(frog))) match {
          case true => restart
          case false => atual
        }
